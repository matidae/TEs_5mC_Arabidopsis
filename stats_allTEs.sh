...
counts(){
    grep -v "#" $1".averageCG.gt4.table" | awk '$6>80' \
        | cut -f9 | cut -f1 -d'/' | sed 's/?//' | sort | uniq -c | sort -k2 > aux1
    counts_all "$1"
    check_counts aux1
    grep -v "#" $1".averageCG.gt4.table" | awk '$6>20 && $6<=80' \
        | cut -f9 | cut -f1 -d'/' | sed 's/?//' | sort | uniq -c | sort -k2 > aux2
    check_counts aux2
    grep -v "#" $1".averageCG.gt4.table" | awk '$6<=20' \
...
values(){
    ...
    for i in ${fams[@]}; do
        n=$((n + 1))
        touch "aux"$n
        grep $i $1.averageCG.gt4.table | cut -f6 > "aux"$n
    done
    if $3; then
...
order=$(awk '{print $9}' $prefix.averageCG.gt4.table | grep -v class | grep "/" \
	| cut -f1 -d'/' | sort | uniq -c | sort -k1nr |awk '{print $2}')
out_prefix=$prefix
values "$out_prefix" "$order" true

fami=$(awk '{print $9}' $prefix.averageCG.gt4.table | grep -v class | grep "/" \
